...
str(mean_male_height) # it is a character
```
...
This returns the first item of cats, which is the coat variables. When using single square bracket`([)` this will return a data.frame with a single variable or column of data. In this case, it returns a character vector, coat.

```{r}
cats[[1]]
```

This also returns the first item of cats, but instead of a data.frame, the use of double square bracket`([[)` returns a character vector.
...
This returns a data.frame containing all variables for the first observations.

...
c(1, 2, 3) will be created as a numeric vector.

c('d', 'e', 'f') will be created as a character vector.

c(1, 2, 'f') will be created as a character vector.
...
The `sep` argument specifies the separator used to join corresponding elements from different vectors. The `collapse` argument specifies the separator used to combine all elements of the resulting vector into a single character string.
...
Use the `read.delim()` function to read tab-delimited text files. It uses `sep = "\t"` to separate columns with tabs and `dec = "."` to indicate a period as the decimal point.
